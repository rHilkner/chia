...
# every plot from before 10-jul-2021 is an OG plot
get_og_plot() {
  dest_dir=$1
  og_plots=( $(ls ${dest_dir} | grep 'plot-k32-2021-07-0') )
  og_plots+=( $(ls ${dest_dir} | grep 'plot-k32-2021-06') )
  og_plots+=( $(ls ${dest_dir} | grep 'plot-k32-2021-05') )
  if (( ${#og_plots[@]} > 0 )); then
    echo "${dest_dir}/${og_plots[0]}"
  else
    echo;
  fi
}
...
get_dest_dir() {
  ...
  for dir in ${dest_array[@]}; do
    ...
    # if drive is busy (in copying_to array), continue
    if $(is_element_in_array ${dir} ${copying_to[@]}); then
      continue
    fi

    # if folder has an OG plot, delete it
    og_plot=$(get_og_plot ${dir})
    if [[ ! -z ${og_plot} ]]; then
      rm ${og_plot}
    fi
  ...
  for i in $(seq ${last_idx} -1 0); do
    ...
    if ! ps -p ${pid} > /dev/null; then
      ...
      if ((total_time < 30)); then
        ...
        filesystem_name=$(df | grep -w "${copying_to[i]}" | awk '{print $1}')
